	...
	for _, v := range accountConfig {
		...
		bam := &BinaAccountManager{
			...
			orderManager: map[common.TransactionID]common.OrderManagerInterface{
				...
				common.MarginID:     systemAgent.NewOrderManager(common.BINANCEID, v.Index, common.MarginID),
				common.CoinFutureID: systemAgent.NewOrderManager(common.BINANCEID, v.Index, common.CoinFutureID),
			},
...
func GetBinanceAccountConfigs(systemAgent common.TradeSystemAgent) []AccountInfo {
	...
	for _, v := range sections {
		if strings.HasPrefix(v.Name(), BinancePluginPrefix) {
