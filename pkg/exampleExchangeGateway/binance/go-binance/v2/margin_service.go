...
// CrossMarginCollateralRatio define isolated margin pair info
...
// IsolatedMarginTransferService transfer assets between spot and isolated margin.
type GetNextHourlyInterestRateService struct {
	c *Client
...
func (s *GetNextHourlyInterestRateService) Assets(assets []string) *GetNextHourlyInterestRateService {
	s.assets = strings.Join(assets, ",")
	return s
}
func (s *GetNextHourlyInterestRateService) IsIsolated(isIsolated bool) *GetNextHourlyInterestRateService {
	s.isIsolated = isIsolated
	return s
}
func (s *GetNextHourlyInterestRateService) Do(ctx context.Context, opts ...RequestOption) (res []*NextHourlyInterestRate, err error) {
	...
	data, err := s.c.callAPI(ctx, r, opts...)
	if err != nil {
		fmt.Println("GetNextHourlyInterestRateService err ", err.Error())
		return nil, err
	}

	res = make([]*NextHourlyInterestRate, 0)
	err = json.Unmarshal(data, &res)
...
type ListMarginDustService struct {
	c *Client
}

// Do sends the request.
func (s *ListMarginDustService) Do(ctx context.Context) (res *[]ListMarginDustResponse, err error) {
	r := &request{
		method:   http.MethodGet,
	...
	common.Logger.Info("ListMarginDustService ", zap.String("raw res", string(data)))

	res = &[]ListMarginDustResponse{}
	err = json.Unmarshal(data, res)
	if err != nil {
		return
	}
...
type MarginDustTransferService struct {
	c      *Client
	assets []string
}

// Asset set asset.
func (s *MarginDustTransferService) Assets(assets []string) *MarginDustTransferService {
	s.assets = assets
	return s
}

// Do sends the request.
func (s *MarginDustTransferService) Do(ctx context.Context) (res *[]ListMarginDustResponse, err error) {
	r := &request{
		method:   http.MethodPost,
		endpoint: "/sapi/v1/margin/exchange-small-liability",
		secType:  secTypeSigned,
	...
	common.Logger.Info("MarginDustTransferService ", zap.String("raw res", string(data)))
