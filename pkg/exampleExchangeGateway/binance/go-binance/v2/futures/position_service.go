...
// ChangeMultiAssetsMarginService change user's position mode
type ChangeMultiAssetsMarginService struct {
	c                *Client
	multiAssetMargin string
}

func (s *ChangeMultiAssetsMarginService) MultiAssetMargin(multiAssetMargin bool) *ChangeMultiAssetsMarginService {
	if multiAssetMargin {
		s.multiAssetMargin = "true"
	} else {
		s.multiAssetMargin = "false"
	}
	return s
}

// Do send request
func (s *ChangeMultiAssetsMarginService) Do(ctx context.Context, opts ...RequestOption) (err error) {
	r := &request{
		method:   http.MethodPost,
	...
	r.setFormParams(params{
		"multiAssetsMargin": s.multiAssetMargin,
	})
	_, _, err = s.c.callAPI(ctx, r, opts...)
	if err != nil {
		return err
